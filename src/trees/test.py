...
class POSModel(nn.Module):
    ...
    def __init__(self, src_model="roberta-base", trg_model="vblagoje/bert-english-uncased-finetuned-pos", freeze=False,
                 max_len=64):
    ...
    def encode_text(self, text):
        ...
        input_ids, attention_mask = torch.empty((0, self.max_len), dtype=torch.long, device=device), torch.empty(
            (0, self.max_len), dtype=torch.long, device=device)
        for each in text:
